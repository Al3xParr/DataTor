...
import { GradeGraphData, Log, TopClimbsGraphData } from './types'
...
export async function getTopClimbsData(logs: Log[]): Promise<TopClimbsDataRtn> {
    const rtn = new Promise<TopClimbsDataRtn>((resolve) => {
        ...
        while (count != logs.length) {
            ...
            const climbsInyear = logs.filter(
                (l) => l.date.getFullYear() == currentYear
            )
            const topClimbForYear = climbsInyear.slice(0, 10)

            topClimbForYear.forEach((l) => {
                names[currentYear + '-' + l.grade] = [
                    ...(names[currentYear + '-' + l.grade] ?? []),
                ].concat([l.name])
        ...
        resolve({
            topClimbsPerYear: topClimbsPerYear.reverse(),
            gradeList,
            names,
        })
    })
...
export async function getTimelineData(climbs: Log[]): Promise<TimelineDataRtn> {
    const rtn = new Promise<TimelineDataRtn>((resolve) => {
        ...
        const gradesOut = presentGrades
            .filter((grade) => grade.startsWith('5.'))
            .sort((a, b) => new GradeConverter().compareGrade(a, b))
        const gradesOutYds = presentGrades
            .filter((grade) => !grade.startsWith('5.'))
            .sort((a, b) => new GradeConverter().compareGrade(a, b))

        resolve({
            data: climbDate,
            presentGrades: gradesOutYds.concat(gradesOut),
        } as TimelineDataRtn)
    })
...
export async function getAvgMaxData(logs: Log[]): Promise<AvgMaxData[]> {
    ...
    while (count != logs.length) {
        ...
        if (yearLogs.length > 0) {
            ...
            const avgList = yearLogs.map((l) =>
                gradeConverter.getGradeIndex(l.grade)
            )
            const avg = Math.floor(
                avgList.reduce((acc, val) => acc + val) / yearLogs.length
            )
            avgMaxData.push({
                year: currentYear,
                max: max,
...
function cleanAreaName(name: string) {
    switch (name) {
        ...
        case 'Herefordshire':
            return 'Herefordshire, County of'
        case 'Isle of Man':
            return 'Man, Isle of'
...
export async function getMapData(logs: Log[]) {
    ...
    counties.forEach((county) => {
        ...
        const climbs = logs
            .filter((log) => cleanAreaName(log.county) == cleanedName)
            .sort((a, b) => gradeConverter.compareLog(a, b))
        const {
            minIndex: min,
            maxIndex: max,
            distribution: gradeDistribution,
        } = gradeConverter.getGradeDistribution(
        ...
        areaFreq[cleanedName] = {
            ...
            topClimbs: climbs
                .slice(0, 3)
                .map((climb) => climb.grade + '/-' + climb.name),
    ...
    countries.forEach((country) => {
        ...
        const climbs = logs
            .filter((log) => log.country == country)
            .sort((a, b) => gradeConverter.compareLog(a, b))
        const {
            minIndex: min,
            maxIndex: max,
        ...
        areaFreq[cleanedName] = {
            ...
            topClimbs: climbs
                ...
                .map((climb) => climb.grade + '/-' + climb.name),
            gradeDistribution: gradeDistribution,
            minGrade: gradeConverter.getGradeFromIndex(
                min,
                (country == 'USA' || country == 'Canada') &&
                    logs[0].type != 'Bouldering'
                    ? 'yds'
                    : scale
            ),
            maxGrade: gradeConverter.getGradeFromIndex(
                max,
                (country == 'USA' || country == 'Canada') &&
                    logs[0].type != 'Bouldering'
                    ? 'yds'
