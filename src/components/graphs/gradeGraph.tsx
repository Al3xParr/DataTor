...
function BarLabel(props: BarLabelProps) {
    const { color, yOrigin, x, y, width, skipAnimation, ...otherProps } = props
    ...
    return <Text {...otherProps} textAnchor="middle" {...animatedProps} />
}
    ...
    function getTotalForGrade(index: number, seriesId: string) {
        ...
        switch (seriesId) {
            case 'repeat':
                return total
            case 'send':
                if (gradeData.repeat == 0) return total
            case 'groundup':
                if (gradeData.send + gradeData.repeat == 0) return total
            case 'flash':
                if (gradeData.groundup + gradeData.send + gradeData.repeat == 0)
                    return total
            case 'onsight':
                if (
                    gradeData.flash +
